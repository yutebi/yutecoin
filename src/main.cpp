...
#if defined(NDEBUG)
# error "Yutecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Yutecoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Yutecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Yutecoin: find block wanted by given orphan block
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexPrev->nHeight <= 2628000) //1-5 years
        nSubsidy = 19.0258 * COIN;
    else if(pindexPrev->nHeight <= 5256000) //5-10 years
        nSubsidy = 9.5129 * COIN;
    else if(pindexPrev->nHeight <= 7884000) //10-15 years
        nSubsidy = 4.7565 * COIN;
    else if(pindexPrev->nHeight <= 10512000) //15-20 years
        nSubsidy = 2.3782 * COIN;
    else //20-30 years
        nSubsidy = 1.1891 * COIN;
...
// Yutecoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Yutecoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Yutecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Yutecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Yutecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Yutecoin: track money supply and mint amount info
...
// Yutecoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Yutecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Yutecoin: compute stake entropy bit for stake modifier
    ...
    // Yutecoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Yutecoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Yutecoin: attempt to generate suitable proof-of-stake
